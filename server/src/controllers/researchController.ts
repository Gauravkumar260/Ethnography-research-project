...
const submitResearch = asyncHandler(async (req: Request, res: Response): Promise<any> => {
  ...
  if (!mainFile) return res.status(400).json({ success: false, message: "Main research document is required." });
  if (!ethicsFile) return res.status(400).json({ success: false, message: "Ethics approval document is required." });

  const requiredFields = ["studentName", "studentId", "email", "program", "mentor", "title", "abstract", "community", "type"];
  for (const field of requiredFields) {
    if (!req.body[field]) return res.status(400).json({ success: false, message: `Field \${field} is required.` });
  }
  ...
  const normalize = (f: any) => f ? path.relative(process.cwd(), f.path).replace(/\\/g, "/") : undefined;
  ...
  const savedResearch = await researchService.create({
    ...
    fileUrl: normalize(mainFile),
    ethicsFileUrl: normalize(ethicsFile),
    mediaFileUrl: normalize(mediaFile),
    status: "pending"
  });

  res.status(201).json({ success: true, message: "Research submitted successfully!", data: savedResearch });
});

const getAllSubmissions = asyncHandler(async (req: Request, res: Response): Promise<any> => {
  const submissions = await researchService.findAll({}, (req as any).user);
  res.status(200).json({ success: true, count: submissions.length, data: submissions });
});

const getPublicResearch = asyncHandler(async (req: Request, res: Response): Promise<any> => {
  const research = await researchService.findAll({}, (req as any).user);
...
const updateStatus = asyncHandler(async (req: Request, res: Response): Promise<any> => {
  ...
  const updatedResearch = await researchService.updateStatus(req.params.id as string, {
    status, rejectionReason: comments || "", reviewedBy: (req as any).user._id, reviewedDate: new Date()
  });
  if (!updatedResearch) return res.status(404).json({ success: false, message: "Research not found" });
  res.status(200).json({ success: true, message: `Research marked as \${status}`, data: updatedResearch });
});
...
const getResearchStats = asyncHandler(async (req: Request, res: Response): Promise<any> => {
  ...
  typeStats.forEach(item => { if (item._id) countsByType[item._id.toLowerCase()] = item.count; });
  const totalCount = await researchService.countAll();
  const approvedCount = await researchService.countApproved();
  const stats = {
    byType: countsByType,
    settlement: [{ label: "Permanent", value: 45, icon: "??" }, { label: "Nomadic", value: 25, icon: "??" }],
...
export { submitResearch, getAllSubmissions, getPublicResearch, updateStatus, getResearchStats };
