...
import { Database, Download, ChevronLeft, Loader2, AlertCircle, FileAudio, Image as ImageIcon, FileText, FileSpreadsheet, File } from 'lucide-react';
import api from '@/lib/api';

// Define the valid types based on your backend
type FilterType = 'all' | 'interview' | 'photo' | 'survey' | 'field_note' | 'document' | 'dataset';

export default function FieldDataPage() {
  const [activeFilter, setActiveFilter] = useState<FilterType>('all');
  const [allData, setAllData] = useState([]); // Store all data here
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // We fetch everything so we can filter client-side instantly
        setAllData(data);
      } catch (error) {
  ...
  const filteredData = allData.filter((item: any) => {
    // Only show relevant field-data types
    const isFieldDataType = ['dataset', 'interview', 'photo', 'survey', 'field_note', 'document'].includes(item.type);
    
    if (!isFieldDataType) return false;

    if (activeFilter === 'all') return true;
    return item.type === activeFilter;
  });

  // 3. Helper for Dynamic Icons
  ...
  const filters: { id: FilterType; label: string; icon: any }[] = [
    ...
    { id: 'field_note', label: 'Field Notes', icon: FileText },
  ];
                  ...
                  <div className="flex flex-col md:flex-row gap-6 items-start">
                    ...
                    <div className="flex-1 w-full">
                      <div className="flex flex-wrap gap-2 mb-3">
                          ...
                          {/* Optional: Show file size if available */}
                          {data.datasetSize && (
                      ...
                      <h3 className="text-[#1a1a1a] mb-2 text-xl font-bold font-serif leading-tight">{data.title}</h3>
                      ...
                      <p className="text-sm text-[#1a1a1a]/70 leading-relaxed mb-6 border-l-2 border-[#99302A]/20 pl-4">
                        {data.abstract}
                      </p>
                      ...
                      <div className="flex gap-4">
                        ...
                        >
                          <Download className="w-4 h-4" /> Download {getTypeLabel(data.type)}
                        </a>
                      </div>
                    </div>
                  </div>
